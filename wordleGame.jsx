...
const WordleGame = () => {
  ...
  //To lowercase for file comparison
  ...
  for (let i = 0; i < TARGET_WORD.length; i++) {
    ...
    } else if (TARGET_WORD.includes(guess[i]) /*&& TARGET_WORD.indexOf(guess[i]) !== i */ && !newMatchedLetters.includes('yellow')) {
  ...
  const renderWord = () => {
    if (!gameOver) {
      return null; // Do not render anything if the game is not over yet
    }
      ...
      <div className="word-display">
        {guess &&
          guess.split('').map((letter, index) => {
            const isCorrect = letter === TARGET_WORD[index];
            const isInWord = TARGET_WORD.includes(letter);
  
            let highlight = null;
            if (isCorrect) {
              highlight = 'green';
            } else if (!isCorrect && isInWord) {
              highlight = 'yellow';
            } else {
              highlight = 'gray';
            }
            ...
            return (
              <span key={index} className={`letter ${highlight}`}>
                {highlight !== 'green' ? letter.toUpperCase() : '_'}
              </span>
            );
          })}
      </div>
